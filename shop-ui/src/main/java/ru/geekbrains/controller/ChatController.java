...
import ru.geekbrains.chat.ChatMessage;
...
public class ChatController {
    ...
    private final SimpMessagingTemplate template; //бин создается автоматически если конфигурация верна
...
//    SimpMessageHeaderAccessor headerAccessor - для работы с http заголовками
//    createHeaders(headerAccessor.getSessionId()) для того чтобы сообщение ушло конкретному пользователю, а не всем
    @MessageMapping("/send_message")
    public void messageReceiver(@Payload ChatMessage chatMessage, SimpMessageHeaderAccessor headerAccessor) {
        if (headerAccessor.getUser() == null) { //если у нас не указан пользователь
